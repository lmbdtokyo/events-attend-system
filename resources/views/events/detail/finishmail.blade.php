...
<script src="https://cdn.quilljs.com/1.3.6/quill.js"></script>
    ...
    <form action="{{ route('eventfinishmail.update', $event->id) }}" method="POST" id="form">
        @csrf
        @method('PATCH')
            ...
            <div class="card-body">
                ...
                <table class="table table-bordered event-setting-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" id="bcc" name="bcc" class="form-control" value="{{ $eventfinishmail->bcc }}">
                            <p class="small">※BCCはカンマ区切りで複数のメールアドレスを入力できます。</p>
                        </td>
                    </tr>
                    <tr>
                        <td><label for="title">メールタイトル</label></td>
                        <td><input type="text" id="title" name="title" class="form-control" value="{{ $eventfinishmail->title }}"></td>
                    </tr>
                    <tr>
    ...
    <script>
        ...
        @if(isset($eventfinishmail) && $eventfinishmail->text)
            quillText.root.innerHTML = `{!! addslashes($eventfinishmail->text) !!}`;
        @endif
